...
/*
  TODO: weird behaviour when the windo is moved from one monitor to another,
        moving a mu window to the top half of the window on my mac's screen causes it to bug out.
*/
...
void r_window_event_callback(SDL_Event *e) {
  switch (e->window.event) {
    case SDL_WINDOWEVENT_RESIZED: SDL_Log("Window %d size changed to %dx%d", e->window.windowID, e->window.data1, e->window.data2); break;
  }
}
#endif
